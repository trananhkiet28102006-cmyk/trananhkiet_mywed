...
<div class="container py-3">
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb bg-white p-3 rounded-4 shadow-sm">
            ...
            <li class="breadcrumb-item active text-primary fw-bold" aria-current="page">{{ $brand->brandname }}</li>
        </ol>
    </nav>
    ...
    <div class="bg-white p-4 rounded-4 shadow-sm mb-4 d-flex justify-content-between align-items-center">
        <div>
            <h3 class="fw-bold mb-1 text-dark">
                <i class="bi bi-award-fill text-primary me-2"></i>Thương hiệu: {{ $brand->brandname }}
            </h3>
            <p class="text-muted mb-0 small">{{ $brand->description ?: 'Các sản phẩm chính hãng thuộc thương hiệu ' . $brand->brandname }}</p>
        </div>
    ...
    @else
        <div class="card border-0 shadow-sm rounded-4 p-5 text-center">
            <i class="bi bi-patch-minus text-muted mb-3" style="font-size: 3.5rem;"></i>
            <h5 class="fw-bold text-dark">Thương hiệu này hiện chưa có sản phẩm!</h5>
            <p class="text-muted">Vui lòng quay lại sau hoặc khám phá các thương hiệu khác.</p>
            <div>
