...
<table class="table table-bordered table-hover table-striped">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                @if($item->image)
                    <img src="{{ str_starts_with($item->image, 'http') ? $item->image : asset('storage/brands/' . $item->image) }}" alt="{{ $item->brandname }}" width="60" class="img-thumbnail">
                @else
            ...
            <td>
                <div class="d-flex gap-1">
                    ...
                    <form action="{{ route('admin.brands.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa thương hiệu này?')">
                        @csrf
                        @method('DELETE')
